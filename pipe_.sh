...
#	printf '\n%s\n' "You have $(zcat "$f"${prefix}R*_001.fastq.gz | echo $((`wc -l`/4))) paired-end raw reads in "$f""
	...
	fastp --in1 "$f"/${prefix}R1_001.fastq.gz  --in2 "$f"/${prefix}R2_001.fastq.gz --out1 "$f"/${prefix}trimmed_R1.fastq.gz --out2 "$f"/${prefix}trimmed_R2.fastq.gz -l 60 -h "$f"/fastp_report.html -w 16
